const contaBancaria = require('../bancodedados');
const {
    format
} = require('date-fns');
const {
...
function consultarContas(req, res) {
    ...
    return res.status(403).json({
        mensagem: "A senha bancária informada é inválida!"
    });
};
...
function criarConta(req, res) {
    ...
    return res.status(201).json({
        mensagem: "Conta crida com sucesso."
    });
};
...
function atualizarUsuario(req, res) {
    ...
    if (!usuarios) {
        return res.status(404).json({
            mensagem: "Não existe usuário com a conta informada"
        });
    }
...
function excluirConta(req, res) {
    ...
    return res.status(403).json({
        mensagem: 'O saldo da conta é maior que R$ 0, por isso, ela não pode ser removida.'
    })
};
...
function depositarDinheiro(req, res) {
    ...
    if (!usuarios) {
        return res.status(404).json({
            mensagem: 'Por favor, informe uma conta existente'
        })
    }
    ...
    return res.status(400).json({
        mensagem: 'Por favor, insira um valor positivo'
    })
};
...
function transferirDinheiro(req, res) {
    ...
    if (valor > contaOrigem.saldo) {
        return res.status(403).json({
            mensagem: 'O valor de transferência é maior que o saldo atual'
        })
    };
    ...
    return res.status(403).json({
        mensagem: 'Saldo insuficiente para esta transferência'
    })
};
...
function saldoDaConta(req, res) {
    ...
    if (!numero_conta || !senha) {
        res.status(404).json({
            mensagem: 'Por favor, insira o número da conta e a senha'
        });
    }
...
function extratoDaConta(req, res) {
    ...
    const depositosEmConta = depositos.filter(deposito => deposito.numero_conta == numero_conta);
    ...
    if (depositosEmConta.length > 0) {
        resposta.depositos = depositos;
    }
    if (transferencias.length > 0) {
        resposta.transferencias = transfRecebidas;
    }
    if (transferencias.length > 0) {
        resposta.transferencias = transfEnviadas;
    }
    if (saquesDaConta.length > 0) {
        resposta.saques = saquesDaConta;
    }
