...
function validarCadastroMiddleware(req, res, next) {
  ...
  if (!cpf) {
    return res.status(400).json({
      mensagem: 'O CPF do cliente é obrigatório'
    });
  }
  if (!email) {
    return res.status(400).json({
      mensagem: 'O email do cliente é obrigatório'
    });
  }
  if (!data_nascimento) {
    return res.status(400).json({
      mensagem: 'A data de nascimento do cliente é obrigatória'
    });
  }
  if (!senha) {
    return res.status(400).json({
      mensagem: 'A senha do cliente é obrigatória'
    });
  }
  next();
}
